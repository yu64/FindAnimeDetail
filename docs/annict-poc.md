...
var condition = new IAnnictClient.SearchCondition(
    List.of("SHIROBAKO", "シャングリラ"), List.of(), null, false);
var works = annictClient.search(condition, annictConfig.channelPriority());
```

条件はタイトル一覧（OR条件）、クール一覧（例: `List.of("2026-autumn")`）、初回日時の下限
（`OffsetDateTime`、未指定はnull）、未来限定（boolean）。タイトルかクールを必須とする。
作品と放送データの先頭ページをまとめて取得する。作品は全ページ取得し、各作品の放送データは最優先局の日時が確定するか末尾まで追加取得する。
まず指定局の優先順で局を選び、その局でAPIに登録されている最初の放送日時を採用する。
過去分を取得前に除外しない。話数・各話タイトルは取得しない。

下限は「以上」、未来限定は検索開始時刻より「後」で判定し、両指定なら両方を満たす作品を返す。
指定局の放送情報がない作品も結果に残す。判明している指定外の局は採用しない。
局名・放送日時の欠損や解釈できない日時はnullとして保持し、日時不明の作品は日時条件でも除外しない。
局が不明な放送枠は、指定局の候補がない場合に使用する。局の優先度は日時の有無より優先する。
初回日時はAsia/Tokyoに変換済み。TSVの放送開始日・時刻・曜日はそこから取り出せる。
返却値は作品ID・作品名・公式URL・初回放送（局ID、局名、日時）。
`FindUsecase` は `IAnnictClient` と `AnnictConfig` を注入して、入力タイトルを重複除去してまとめてOR検索する。TSV・Markdownともに日本時間の曜日（月→日）、時刻の順で並べ、放送開始日は比較に使わない。日時不明は末尾に置き、同じ曜日・時刻の作品同士はAPIの返却順を維持する。
同じ作品IDは1行にまとめ、日本時間の「放送開始日・時刻・曜日・放送局・タイトル・公式URL」を
ヘッダー付きTSVで返す。既定では放送日時・放送局が揃った作品だけを出力する。
`/find #word 作品名 #all`（`FindInput.all` がtrue）の場合は不明な情報がある作品も出力し、不明な欄は `N/A` とする。公式URLの有無は除外条件に含めない。
`#all` は検索語・日時・指定局の条件を解除するものではない。日時不明の作品は `#all` 指定時に残す。0作品の場合はヘッダーのみ返す。
外部の局名・タイトルに含まれるタブ・改行は空白に置き換える。
公式URLが未登録の場合は最終列を空欄にする。
`FindInput.fmt` が `MD` の場合は件数の見出しと「作品・初回放送・放送局・公式サイト」の4列の表を生成し、Kitware Markdown ViewerのURLを返す。作品名はAnnictへのリンクとし、日時は1列にまとめる。
...
`#from 2026-10-15` は日本時間の当日00:00、`#from 2026-10` は同月1日00:00に補完する。
